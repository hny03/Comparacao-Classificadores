...
### 4.1.2. Redução de Dimensionalidade
...
## 4.2. Estratégia da Divisão da Base de Dados
...
form_dt_bean <- reformulate(setdiff(names(train_dt_bean), target_col), response = target_col)
...
cm_001_dt_bean <- confusionMatrix(pred_001_dt_bean, test_dt_bean[[target_col]])
cm_0005_dt_bean <- confusionMatrix(pred_0005_dt_bean, test_dt_bean[[target_col]])

acc_001_dt_bean  <- cm_001_dt_bean$overall["Accuracy"]
acc_0005_dt_bean <- cm_0005_dt_bean$overall["Accuracy"]

cat("\n---------------------- Árvore (cp = 0.01) ----------------------\n")
print(cm_001_dt_bean)
cat(sprintf("\nAcurácia (cp=0.01): %.4f\n", acc_001_dt_bean))

cat("\n--------------------- Árvore (cp = 0.005) ----------------------\n")
print(cm_0005_dt_bean)
...
y_train_knn_bean <- train_knn_bean[[target_col]]
y_test_knn_bean <- test_knn_bean[[target_col]]
...
# Executar KNN para k = 10
k_10_knn_bean <- 10 # Define a quantidade de K-vizinhos mais próximos

pred_k_10_knn_bean <- knn(
  train = X_train_knn_bean,   # base de treino (preditores)
  test  = X_test_knn_bean,    # base de teste  (preditores)
  cl    = y_train_knn_bean,   # classes de treino (factor)
  k     = k_10_knn_bean       # número de vizinhos
)
```
...
idx_teste_rl_bean <- bean_df_pad %>% 
  anti_join(idx_treino_rl_bean)

model_rl_bean <- multinom(Class ~ AspectRation + Eccentricity + EquivDiameter + Extent + Solidity + roundness + Compactness + ShapeFactor1 + ShapeFactor2 + ShapeFactor3 + ShapeFactor4, data = idx_treino_rl_bean)
 
coefs <- summary(model_rl_bean)$coefficients # pesos atribuidos a cada atributo de acordo com a classe
 
prediction_rl_bean <- predict(model_rl_bean, newdata = idx_teste_rl_bean)
```
...
# Avaliar | k = 10
conf_k_10_knn_bean <- table(Predito = pred_k_10_knn_bean, Real = y_test_knn_bean)
acc_k_10_knn_bean  <- mean(pred_k_10_knn_bean == y_test_knn_bean)
...
conf_rl_bean <- table(Predito = prediction_rl_bean, Real = idx_teste_rl_bean$Class)
acc_rl_bean  <- mean(prediction_rl_bean == idx_teste_rl_bean$Class)
...
## 5.2. Estratégia da Divisão da Base de Dados
...
# Fórmula: alvo ~ todos os preditores
form_dt_glass <- reformulate(setdiff(names(train_dt_glass), target_col), response = target_col)
...
# Avaliar (matriz de confusão e acurácia)
cm_001_dt_glass  <- confusionMatrix(pred_001_dt_glass,  test_dt_glass[[target_col]])
cm_0005_dt_glass <- confusionMatrix(pred_0005_dt_glass, test_dt_glass[[target_col]])

acc_001_dt_glass  <- cm_001_dt_glass$overall["Accuracy"]
acc_0005_dt_glass <- cm_0005_dt_glass$overall["Accuracy"]

cat("\n---------------------- Árvore (cp = 0.01) ----------------------\n")
print(cm_001_dt_glass)
cat(sprintf("\nAcurácia (cp=0.01): %.4f\n", acc_001_dt_glass))

cat("\n--------------------- Árvore (cp = 0.005) ----------------------\n")
print(cm_0005_dt_glass)
cat(sprintf("\nAcurácia (cp=0.005): %.4f\n", acc_0005_dt_glass))
```
...
# Matriz de preditores (x) e vetores de classe (y)
X_train_knn_glass <- train_knn_glass %>% select(-all_of(target_col)) %>% as.data.frame()
X_test_knn_glass  <- test_knn_glass  %>% select(-all_of(target_col)) %>% as.data.frame()

y_train_knn_glass <- train_knn_glass[[target_col]]
y_test_knn_glass  <- test_knn_glass[[target_col]]
...
# Avaliar (matriz de confusão e acurácia)
conf_k_5_knn_glass <- table(Predito = pred_k_5_knn_glass, Real = y_test_knn_glass)
...
# Executar KNN para k = 10
k_10_knn_glass <- 10 # Define a quantidade de K-vizinhos mais próximos

pred_k_10_knn_glass <- knn(
  train = X_train_knn_glass,
  test  = X_test_knn_glass,
  cl    = y_train_knn_glass,
  k     = k_10_knn_glass
...
# Avaliar (matriz de confusão e acurácia)
conf_k_10_knn_glass <- table(Predito = pred_k_10_knn_glass, Real = y_test_knn_glass)
acc_k_10_knn_glass  <- mean(pred_k_10_knn_glass == y_test_knn_glass)
...
idx_teste_rl_glass <- glass_df_pad %>% 
  anti_join(idx_treino_rl_glass)
model <- multinom(Class ~ RI + Na + Mg + Al + Si + K + Ca + Ba + Fe, data = idx_treino_rl_glass)
 
coefs <- summary(model)$coefficients # pesos atribuidos a cada atributo de acordo com a classe
 
prediction_rl_glass <- predict(model, newdata = idx_teste_rl_glass)
```
