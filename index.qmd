...
form <- reformulate(setdiff(names(train_dt_bean), target_col), response = target_col)

# Treinar duas árvores com cp diferentes
tree_cp_001  <- rpart(form, data = train_dt_bean, method = "class",
                      control = rpart.control(cp = 0.01))
tree_cp_0005 <- rpart(form, data = train_dt_bean, method = "class",
                      control = rpart.control(cp = 0.005))

# Prever no conjunto de teste
pred_001  <- predict(tree_cp_001,  newdata = test_dt_bean, type = "class")
pred_0005 <- predict(tree_cp_0005, newdata = test_dt_bean, type = "class")

# Avaliar (matriz de confusão e acurácia)
cm_001  <- confusionMatrix(pred_001,  test_dt_bean[[target_col]])
cm_0005 <- confusionMatrix(pred_0005, test_dt_bean[[target_col]])
...
# Fórmula: alvo ~ todos os preditores
form <- reformulate(setdiff(names(train_dt_glass), target_col), response = target_col)

# Treinar duas árvores com cp diferentes
tree_cp_001  <- rpart(form, data = train_dt_glass, method = "class", control = rpart.control(cp = 0.01))
tree_cp_0005 <- rpart(form, data = train_dt_glass, method = "class", control = rpart.control(cp = 0.005))

# Prever no conjunto de teste
pred_001  <- predict(tree_cp_001,  newdata = test_dt_glass, type = "class")
pred_0005 <- predict(tree_cp_0005, newdata = test_dt_glass, type = "class")

# Avaliar (matriz de confusão e acurácia)
cm_001  <- confusionMatrix(pred_001,  test_dt_glass[[target_col]])
cm_0005 <- confusionMatrix(pred_0005, test_dt_glass[[target_col]])
...
y_test  <- test_knn_bean[[target_col]]
...
# Avaliar (matriz de confusão e acurácia)
conf_k <- table(Predito = pred_k, Real = y_test_knn_bean)
acc_k  <- mean(pred_k == y_test_knn_bean)
...
# Executar KNN
k_10 <- 10 # Define a quantidade de K-vizinhos mais próximos
...
# Avaliar (matriz de confusão e acurácia)
conf_k <- table(Predito = pred_k, Real = y_test_knn_glass)
acc_k  <- mean(pred_k == y_test_knn_glass)
