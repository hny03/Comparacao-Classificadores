...
model <- multinom(Class ~ AspectRation + Eccentricity + EquivDiameter + Extent + Solidity + roundness + Compactness + ShapeFactor1 + ShapeFactor2 + ShapeFactor3 + ShapeFactor4, data = idx_treino_rl_bean)
...
prediction <- predict(model, newdata = idx_teste_rl_bean)

idx_teste_rl_bean$Class

head(prediction)

# Avaliar (matriz de confusão e acurácia)
cm_rl_bean <- table(Predito = prediction, Real = idx_teste_rl_bean$Class)
acc_rl_bean <- mean(prediction == idx_teste_rl_bean$Class)
...
prediction <- predict(model, newdata = idx_teste_rl_glass)

idx_teste_rl_glass$Class

head(prediction)

# Avaliar (matriz de confusão e acurácia)
cm_rl_glass <- table(Predito = prediction, Real = idx_teste_rl_glass$Class)
acc_rl_glass <- mean(prediction == idx_teste_rl_glass$Class)
